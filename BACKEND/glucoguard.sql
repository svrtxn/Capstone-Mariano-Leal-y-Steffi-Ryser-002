...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 18-11-2025 a las 00:19:29
...
CREATE TABLE `contactosapoyo` (
  `contacto_id` int(10) UNSIGNED NOT NULL COMMENT 'ID del contacto',
  `usuario_id` int(10) UNSIGNED NOT NULL COMMENT 'Usuario paciente dueño del contacto',
  `contacto_usuario_id` int(10) UNSIGNED DEFAULT NULL COMMENT 'Usuario contacto registrado',
  `nombre_contacto` varchar(100) DEFAULT NULL COMMENT 'Nombre si es externo',
  `email_contacto` varchar(100) DEFAULT NULL COMMENT 'Email si es externo',
  `telefono_contacto` varchar(20) DEFAULT NULL COMMENT 'Teléfono si es externo',
  `tipo_contacto` enum('amigo','medico','familiar','otro') NOT NULL DEFAULT 'amigo' COMMENT 'Tipo de contacto',
  `prioridad` tinyint(3) UNSIGNED NOT NULL DEFAULT 1 COMMENT 'Prioridad',
  `habilitado` tinyint(1) NOT NULL DEFAULT 1 COMMENT 'Habilitado para alertas',
  `fecha_creacion` timestamp NOT NULL DEFAULT current_timestamp() COMMENT 'Fecha de creación'
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='Contactos de apoyo para recibir alertas';
...
CREATE TABLE `usuarios` (
  ...
  `tipo_diabetes` enum('1','2') DEFAULT NULL
...
ALTER TABLE `contactosapoyo`
  ...
  ADD KEY `idx_contacto_usuario` (`usuario_id`),
  ADD KEY `contacto_usuario_id` (`contacto_usuario_id`);
...
--
ALTER TABLE `alertas`
  MODIFY `alerta_id` int(10) UNSIGNED NOT NULL AUTO_INCREMENT COMMENT 'ID de alerta';
...
--
ALTER TABLE `contactosapoyo`
  MODIFY `contacto_id` int(10) UNSIGNED NOT NULL AUTO_INCREMENT COMMENT 'ID del contacto';
...
--
ALTER TABLE `nivelesglucosa`
  MODIFY `glucosa_id` int(10) UNSIGNED NOT NULL AUTO_INCREMENT COMMENT 'Identificador único de la medición de glucosa';

--
-- AUTO_INCREMENT de la tabla `usuarios`
--
ALTER TABLE `usuarios`
  MODIFY `usuario_id` int(10) UNSIGNED NOT NULL AUTO_INCREMENT COMMENT 'Identificador único del usuario';
...
--
-- Filtros para la tabla `contactosapoyo`
--
ALTER TABLE `contactosapoyo`
  ADD CONSTRAINT `contactosapoyo_ibfk_1` FOREIGN KEY (`usuario_id`) REFERENCES `usuarios` (`usuario_id`) ON DELETE CASCADE ON UPDATE CASCADE,
  ADD CONSTRAINT `contactosapoyo_ibfk_2` FOREIGN KEY (`contacto_usuario_id`) REFERENCES `usuarios` (`usuario_id`) ON DELETE SET NULL ON UPDATE CASCADE;
