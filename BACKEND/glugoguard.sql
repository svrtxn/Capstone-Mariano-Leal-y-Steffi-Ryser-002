...
-- Agregar columna tiene_sensor a la tabla Usuarios
ALTER TABLE Usuarios
ADD COLUMN tiene_sensor TINYINT(1) NOT NULL DEFAULT 0 COMMENT 'Indica si el usuario tiene sensor (1: sí, 0: no)';

-- actualizar usuarios existentes si quieres asignar un valor por defecto
UPDATE Usuarios
SET tiene_sensor = 0
WHERE tiene_sensor IS NULL;

-- recuperar clave
ALTER TABLE Usuarios
ADD COLUMN resetToken VARCHAR(255) DEFAULT NULL,
ADD COLUMN resetExpires BIGINT DEFAULT NULL;


-- agregar tipo diabetes 
ALTER TABLE Usuarios
ADD COLUMN tipo_diabetes ENUM('1', '2') DEFAULT NULL;
